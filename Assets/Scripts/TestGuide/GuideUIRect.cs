...
/// 矩形遮罩镂空引导
/// </summary>
public class GuideUIRect : MonoBehaviour,IUIGuide
{
    ...
    private void OnEnable()
    {
        ...
        //计算高亮显示区域咋画布中的范围
        _targetOffsetX = Vector2.Distance(WorldToCanvasPos(CurrentCanvas, _corners[0]), WorldToCanvasPos(CurrentCanvas, _corners[3])) / 2f;
        _targetOffsetY = Vector2.Distance(WorldToCanvasPos(CurrentCanvas, _corners[0]), WorldToCanvasPos(CurrentCanvas, _corners[1])) / 2f;
        //计算高亮显示区域的中心
        ...
        if (canvasRectTransform != null)
        {
            ...
            {
                if (i % 2 == 0)
                    _currentOffsetX = Mathf.Max(Vector3.Distance(WorldToCanvasPos(CurrentCanvas, _corners[i]), center), _currentOffsetX);
                else
                    _currentOffsetY = Mathf.Max(Vector3.Distance(WorldToCanvasPos(CurrentCanvas, _corners[i]), center), _currentOffsetY);
            }
        }
        //设置遮罩材质中当前偏移的变量
    ...
    public void SetTarget(RectTransform target, Canvas canvas)
    {
        Target = target;
