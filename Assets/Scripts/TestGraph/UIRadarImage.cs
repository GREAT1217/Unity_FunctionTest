...
public class UIRadarImage : MaskableGraphic
{
    ...
    private void DrawTriangle(VertexHelper vh, int index)
    {
        int nextIndex = index + 1 >= _datas.Length ? 0 : index + 1;
        float radian1 = index * _perRadian;
        float radian2 = nextIndex * _perRadian;
    ...
    /// <param name="vh"></param>
    /// <param name="index"></param>
    private void DrawLine(VertexHelper vh, int index)
    {
        int nextIndex = index + 1 >= _datas.Length ? 0 : index + 1;
        float radian1 = index * _perRadian;
        float radian2 = nextIndex * _perRadian;
        ...
        Vector3 p2 = new Vector3(radius2 * Mathf.Cos(radian2), radius2 * Mathf.Sin(radian2));
        //画线
        UIVertex[] newVertexs = GetQuad(p1, p2, _lineColor, _lineWidth);
